...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < CONNECTION_SIZE; ++i) {
        ...
        // copy client_socket
        int client_socket2 = dup(client_socket);
        writefp = fdopen(client_socket2, "w");
        ...
        // invoke `shutdown` function via copied file descriptor `client_socket2`,
        // the EOF will be sent to the client.
        shutdown(client_socket2, SHUT_WR);
        // destroy all file descriptors, then the socket will be destroyed.
        fclose(writefp);

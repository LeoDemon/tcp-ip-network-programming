...
int thread_dispatch() {
    ...
    printf("sum==[%ld], end main thread: %ld for process: %d...\n", sum, pthread_self(), getpid());
    return 0;
}

int main(int argc, char *argv[]) {
    // 初始化互斥量（mutual exclusion）
    int mutex_ret = pthread_mutex_init(&mutex, NULL);
    //mutex = PTHREAD_MUTEX_INITIALIZER;
    if (0 != mutex_ret) {
        puts("init mutex failed");
        return 1;
    }
...
void *thread_inc(void *arg) {
    ...
    sprintf(result_msg, "thread_inc.sum==%ld", sum);
...
void *thread_des(void *arg) {
    ...
    char *result_msg = (char *) malloc((sizeof(char) * BUFFER_SIZE));
    bzero(result_msg, BUFFER_SIZE);
    sprintf(result_msg, "thread_des.sum==%ld", sum);

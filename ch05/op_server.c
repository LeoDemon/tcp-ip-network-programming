...
int main(int argc, char **argv) {
    ...
    for (int i = 0; i < CONNECTION_SIZE; ++i) {
        ...
        while (1) {
            read_len = read(client_socket, &op_num, OP_PARAM_SIZE);
            ...
            while (op_recv_len < op_total_len) {
                read_len = read(client_socket, &message[op_recv_len / OP_PARAM_SIZE], op_total_len);
                op_recv_len += read_len;
            }
//            message[op_num + 1] = '0';

            bzero(cal_result, sizeof(cal_result));
            calculate(op_num, message, message[op_num], cal_result);
